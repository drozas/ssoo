        ...
        LOOP
                        ...
                        CASE pid  IS
                           ...
                           WHEN 0 =>
                              ...
                              IF title="/kill" THEN --si recibimos la orden de matar al padre
                				
                				pid_aux:=GETPPID; --guardamos el id del padre              	
                              	res_aux:=KILL(pid_aux,SIGTERM); --le invitamos a morirse
                              	IF res_aux=-1 THEN
                              		TXT.PUT_LINE("Error al intentar que el proceso padre se suicidara");
                              	END IF;
                              	
                              	res_aux:=SLEEP(2); --dormimos dos sg
                              	
                              	res_aux:=KILL(pid_aux,SIGKILL); --si no se suicido, lo matamos
                              	IF res_aux=-1 THEN
                              		TXT.PUT_LINE("Error al intentar matar inmediatamente al proceso padre");
                              	END IF;
