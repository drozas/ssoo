...
-- Fase actual: fase 11 (entrega-Septiembre)
-- Autor: David Rozas
-- ***********************************************************
...
-- ***********************************************************************************************
procedure VerError (Num_Error: in Integer) is
begin
   if Num_Error=-1 then
...
function DameTitle (direccion: in InfoPeticion_Type) return ASU.Unbounded_String is
...
procedure MatarPadre (Pid:in Integer) is
   Res_Aux:Integer;
begin
...
procedure TratarStat(Conex_Fd:in integer) is
...
procedure MeterDirectorio(Directorio_Fd: in Dirhandle; Cache: in TPuntCache; Tam : in out integer) is
...
function PosicionSiguiente (Pos: in Integer) return Integer is
Pos_Aux: Integer;
begin
   Pos_Aux:= Pos mod N_PAGS + 1;
   return Pos_Aux;
...
function EsCacheVacia(pCache: in TPuntCache) return Boolean is
EsVacia:Boolean;
begin
...
function EsCacheLLena(pCache: in TPuntCache) return Boolean is
EsLLena:Boolean;
begin
...
function EstaEnCache(PagABuscar: in ASU.Unbounded_String; pCache: in TPuntCache) return integer is
Encontrada:Boolean;
I:Integer;
Posicion: Integer;
begin
...
procedure Leer_Pagina(NombreFichero: in ASU.Unbounded_String; pCache: in TPuntCache;
                      Title : in ASU.Unbounded_String; Salida: out ASU.Unbounded_String; Tam :  out integer) is
