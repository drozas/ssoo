...
procedure MostrarDirectorio(Directorio_Fd:Dirhandle; Salida_Fd:integer) is
   Directorio_Actual: ASU.Unbounded_String;
   Res_Aux:Integer;
begin
   ...
   WHILE directorio_actual /="" LOOP
      IF directorio_actual /="." AND directorio_actual/=".." THEN
         res_aux:= WRITE(Salida_fd, ASU.to_string(directorio_actual)& Ascii.lf, ASU.length(directorio_actual)+1);
         VerError(Res_Aux);
      END IF;
      directorio_actual:=ASU.to_unbounded_string(READDIR(directorio_fd));
   END LOOP;
end MostrarDirectorio;
                 ...
                 ELSE --si no, intentamos abrirlo
                    ...
                    ELSE
                       ...
                       IF ERRNO = EISDIR THEN -- si es un direct., lo abrimos y mostramos su contenido por el navegador
                          ...
                          MostrarDirectorio(Directorio_Fd, Conex_fd);
